---
title: 작업 규칙 (Workflow Rules)
date: 2026-01-19
---

# 작업 규칙 (Workflow Rules)

## 규칙 1: 모든 작업은 History에 기록 (필수)

**중요:** 사용자와 함께 수행한 모든 작업은 반드시 다음 두 곳에 기록되어야 합니다:

### 1.1 로컬 작업 로그 업데이트
- 파일: `<project-root>/work_log.txt`
- 형식: `[날짜 시간] 작업 내용 설명`
- 예시: `[2026-01-19 16:30:46] 작업 규칙 파일 생성 및 누락된 History 업데이트`

### 1.2 블로그 History 포스트 업데이트
- 파일: `<project-root>/tech-blog/posts/Settings/project-history.md`
- 형식: `- **시간**: 작업 내용 설명 \`[AI 모델명]\``
- 예시: `- **16:30:46**: 작업 규칙 파일 생성 및 누락된 History 업데이트 \`[Claude Sonnet 4.5]\``

### 1.3 GitHub에 Push
- 모든 History 업데이트는 즉시 GitHub에 Push하여 웹페이지에서 확인 가능하도록 함
- 명령어: `git add posts/Settings/project-history.md && git commit -m "Update history" && git push`

### 1.4 작업 완료 후 블로그 링크 제공
- GitHub Push가 완료되면 사용자가 확인할 수 있도록 **클릭 가능한 블로그 링크**를 반드시 제공
- 형식: `👉 **블로그 주소:** [https://androidsta01.github.io/tech-blog/](https://androidsta01.github.io/tech-blog/)`
- 배포 완료까지 약 1분 소요됨을 안내

## 규칙 2: 민감한 정보 보호

**절대 기록하지 말 것:**
- 계정 ID/사용자명
- 비밀번호
- API 토큰
- GitHub Personal Access Token
- 이메일 주소
- 기타 개인 식별 정보

**History 기록 시:**
- 민감한 정보는 일반적인 표현으로 대체
- 예시: "GitHub 계정 연동 완료" (계정명 명시 안 함)
- 예시: "인증 토큰 설정 완료" (토큰 값 명시 안 함)

## 규칙 3: 파일 저장 위치

**사용자 작업 파일:**
- 모든 프로젝트 파일은 `<project-root>` 또는 하위 폴더에 저장
- **절대 금지**: `.gemini/antigravity/brain/` 폴더에 사용자용 문서 저장
- Brain 폴더는 시스템 메모리(task.md, walkthrough.md 등)만 저장

**예외:**
- `task.md`, `implementation_plan.md`, `walkthrough.md`는 brain 폴더에 저장 (시스템 파일)

## 규칙 4: 명령어 실행 전 승인

**자동 실행 가능:**
- 파일 읽기/조회
- 안전한 명령어 (ls, cat, git status 등)
- 명시적으로 SafeToAutoRun=true로 표시된 명령어

**사용자 승인 필요:**
- 파일 삭제
- 시스템 설정 변경
- 패키지 설치
- sudo 권한 필요한 작업
- 외부 API 호출

## 규칙 5: 진행 상황 표시

**작업 진행 시:**
- 가능한 경우 진행률(%) 표시
- 예시: "[50%] 패키지 설치 중..."
- 사용자가 작업 상태를 명확히 알 수 있도록 함

**멈춤 방지:**
- 오래 걸리는 작업은 중간 상태 업데이트
- 사용자가 취소 버튼을 누르지 않도록 명확한 피드백 제공

## 규칙 6: AI 모델 표기 및 정렬

**History 기록 원칙:**
1. **정확한 모델명 기입 (중요):**
   - 현재 작업 중인 AI 모델의 이름을 정확히 기입할 것
   - 예: `[Gemini 3 Pro]`, `[Claude Sonnet 4.5]` 등
   - **주의:** 자신이 수행한 작업에 다른 모델명을 적지 말 것

2. **시간순 정렬 (Chronological Order):**
   - 모든 기록은 **오래된 순서에서 최신 순서**로 정렬 (과거 -> 현재)
   - 중간에 끼워넣지 말고, 시간이 흐른 순서대로 작성

**형식:**
```markdown
- **16:30:46**: 작업 내용 설명 `[Gemini 3 Pro]`
```

## 규칙 7: 화면 녹화 제한

**기본 원칙:**
- 사용자가 명시적으로 요청하지 않는 한 화면 녹화 금지
- 이유: 시스템 성능 저하 (렉 발생)

**예외:**
- 사용자가 "화면 녹화해줘" 또는 "과정을 보여줘" 등 명시적 요청 시에만 수행

## 작업 체크리스트

매 작업마다 다음을 확인:

- [ ] 작업 완료 후 `work_log.txt` 업데이트
- [ ] `project-history.md`에 작업 내역 추가 (AI 모델명 포함)
- [ ] 민감한 정보(ID, 토큰, 비밀번호) 제거 확인
- [ ] GitHub에 Push하여 웹페이지 반영
- [ ] 필요시 `task.md` 업데이트
- [ ] 파일이 올바른 위치에 저장되었는지 확인

## 예외 사항

다음의 경우는 History 기록 생략 가능:
- 단순 질문 응답 (코드 수정 없음)
- 파일 조회만 수행
- 사용자가 명시적으로 기록 불필요하다고 언급한 경우

## 규칙 8: Posting 카테고리 작성 스타일

**Posting 폴더의 글 작성 규칙:**
- 깔끔하고 간결한 한글로 작성 (~습니다/~합니다 체)
- 고유명사를 제외한 모든 영어 단어는 한글로 번역
- 자연스럽고 읽기 편한 문체로 작성
- 기술 용어도 가능한 한글로 풀어서 설명

**메타데이터 필수 포함:**
- 작성 날짜 (frontmatter의 `date` 필드)
- 사용한 AI 모델 (글 하단에 명시)

**예시:**
- ❌ "Deploy를 진행합니다" → ✅ "배포를 진행합니다"
- ❌ "Performance가 좋습니다" → ✅ "성능이 좋습니다"
- ✅ "GitHub Pages" (고유명사는 그대로 유지)
- ✅ "Next.js" (고유명사는 그대로 유지)

**포스팅 파일 형식:**
```markdown
---
title: 글 제목
date: 2026-01-19```

## 규칙 9: 카테고리 및 사이드바 구조

**계층 구조:**
- **Posting**: 최상위 카테고리 (헤더 역할)
  - **Blog**: 하위 카테고리 (실제 글이 위치)
  - **AI**, **Unity** 등: 향후 추가될 하위 카테고리
- **Settings**: 최상위 카테고리 (헤더 역할 및 단독 메뉴)

**디자인 원칙:**
- **Posting**과 **Settings**는 동일한 위계(크기, 색상, 굵기)로 표현
- 하위 카테고리(Blog 등)는 들여쓰기와 작은 폰트로 위계 구분
- 중복 카테고리가 표시되지 않도록 렌더링 로직 주의
---

## 규칙 10: 제목 일관성 (Title Consistency)

**필수 규칙:**
- Frontmatter의 `title` 필드와 본문의 첫 번째 `# 제목`(H1)이 **완전히 일치**해야 합니다.
- 사용자가 클릭 전에 보는 제목과 클릭 후 본문에서 보는 제목이 동일해야 합니다.

**올바른 예시:**
```markdown
---
title: Unity 6와 2025년 로드맵 심층 분석
date: 2026-01-19
---

# Unity 6와 2025년 로드맵 심층 분석

본문 내용...
```

**잘못된 예시:**
```markdown
---
title: Unity 6와 2025년 로드맵 심층 분석
date: 2026-01-19
---

# Unity 6: 렌더링 성능과 생산성의 도약  ❌ (제목 불일치)

본문 내용...
```

**주의사항:**
- 이모지, 특수문자, 공백까지 정확히 일치해야 합니다.
- 부제목이 필요한 경우 H1 아래에 일반 텍스트나 H2로 작성하세요.

---

# 작업 규칙 (Workflow Rules)

본문 내용...

---
*이 글은 Claude Sonnet 4.5를 사용하여 작성되었습니다.*
```
